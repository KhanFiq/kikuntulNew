                    ...
                    {order.items.map((item, idx) => (
                      <div key={idx} className="flex flex-col gap-1">
                        <div className="flex items-center gap-1">
                          ...
                          <span>{item.product?.name || <span className="italic text-gray-400">(nama tidak tersedia)</span>}</span>
                          <span className="text-xs text-gray-400">x{item.quantity}</span>
                        </div>
                        <div className="text-xs text-purple-700 font-bold pl-6">
                          {typeof item.product?.price === 'number' ? `Rp${item.product.price.toLocaleString()}` : <span className="italic text-gray-400">(harga tidak tersedia)</span>}
                        </div>
                      </div>
                    ))}
                  </td>
